...
public class YooPageDTO {
	...
	private YooCriteria cri;	// 페이지번호(pageNum)와 수량(amount)을 하나의 객체로 표한한  cri 필드 선언!
	
	public YooPageDTO(YooCriteria cri, int total) {
		this.cri = cri;
		this.total = total;
		
		// 페이징 끝 번호(endPage) 계산식을 적용합니다.
